...
function PastProject() {
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      try {
        const response = await axios.get("http://localhost:3001/jeevanathi");
          ...
          <Card key={index} className="card">
            <CardContent className="cardContent">
              ...
              <Grid container spacing={2} justifyContent="center" className="photoGrid">
                {event.img.slice(0, visiblePhotos).map((img, photoIndex) => (
              ...
              {event.img.length > maxVisiblePhotos && (
                <div className="buttonContainer">
                  {visiblePhotos < event.img.length ? (
                    <>
                      <button
